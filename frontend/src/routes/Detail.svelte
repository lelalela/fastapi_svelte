<script>
    ...
    function post_delete(question_id) {
        ...
        fastapi('delete', '/api/question/delete', params, 
            (json) => {
                alert('삭제가 완료되었습니다.');
                push('/');
            },
    ...
    function delete_answer(answer_id) {
        ...
        fastapi('delete', '/api/answer/delete', params,
        (json) => {
            question = json
        }, 
        (error) => {
        ...
        <div class="card-body">
            ...
            <div class="my-3">
                {#if question.user && $username === question.user.username }
                    ...
                    <button class="btn btn-sm btn-outline-secondary" on:click={() => post_delete(question.id)}>삭제</button>
                {/if}
            </div>
        </div>
    </div>
